...
public abstract class TileEntityElectrical extends TileEntityAdvanced implements
        IElectrical, IElectricalStorage {
    ...
    public void recharge(ItemStack itemStack) {
        this.setEnergyStored(this.getEnergyStored()
                - ElectricItemHelper.chargeItem(itemStack,
                        this.getProvide(ForgeDirection.UNKNOWN)));
    }
    ...
    public void discharge(ItemStack itemStack) {
        this.setEnergyStored(this.getEnergyStored()
                + ElectricItemHelper.dischargeItem(itemStack,
                        this.getRequest(ForgeDirection.UNKNOWN)));
    }
    ...
    public void produceUE(ForgeDirection outputDirection) {
        if (!this.worldObj.isRemote && outputDirection != null
            ...
            if (provide > 0) {
                ...
                IElectricityNetwork outputNetwork = ElectricityHelper
                        .getNetworkFromTileEntity(outputTile, outputDirection);

                if (outputNetwork != null) {
                    ElectricityPack powerRequest = outputNetwork
                            .getRequest(this);

                    if (powerRequest.getWatts() > 0) {
                        ElectricityPack sendPack = ElectricityPack.min(
                                ElectricityPack.getFromWatts(
                                        this.getEnergyStored(),
                                        this.getVoltage()),
                                ElectricityPack.getFromWatts(provide,
                                        this.getVoltage()));
                        float rejectedPower = outputNetwork.produce(sendPack,
                                this);
                        this.provideElectricity(sendPack.getWatts()
                                - rejectedPower, true);
                    }
                }
            }
     ...
     * 
     * @return The direction that electricity is entered into the tile. Return
     *         null for no input. By default you can accept power from all
     *         sides.
     */
     ...
     * 
     * @return The direction that electricity is output from the tile. Return
     *         null for no output. By default it will return an empty EnumSet.
     */
    ...
    public float receiveElectricity(ElectricityPack receive, boolean doReceive) {
        if (receive != null) {
            ...
            float newStoredEnergy = Math.min(
                    this.getEnergyStored() + receive.getWatts(),
    ...
    public float receiveElectricity(float energy, boolean doReceive) {
        return this.receiveElectricity(
                ElectricityPack.getFromWatts(energy, this.getVoltage()),
                doReceive);
    }
     ...
     * use it internally.
     */
    public ElectricityPack provideElectricity(ElectricityPack request,
            boolean doProvide) {
        if (request != null) {
            float requestedEnergy = Math.min(request.getWatts(),
                    this.energyStored);
            ...
            return ElectricityPack.getFromWatts(requestedEnergy,
                    this.getVoltage());
        }
    ...
    public ElectricityPack provideElectricity(float energy, boolean doProvide) {
        return this.provideElectricity(
                ElectricityPack.getFromWatts(energy, this.getVoltage()),
                doProvide);
    }
    ...
    public void setEnergyStored(float energy) {
        this.energyStored = Math.max(
                Math.min(energy, this.getMaxEnergyStored()), 0);
    }
    ...
    public boolean canConnect(ForgeDirection direction) {
        ...
        return this.getInputDirections().contains(direction)
                || this.getOutputDirections().contains(direction);
    }
