        ...
        if (itemstack != null && player != null && entity != null) {
            if (!player.isSneaking()) {
                ...
                if (entity instanceof EntityHorse || entity instanceof EntityGhast)
                    return false;
                if (mount != null && !mount.equals(entity) && !mount.isRiding() && !entity.isRiding() && mount.riddenByEntity == null && entity.riddenByEntity == null) {
                    entity.mountEntity(mount);
                    tick++;
                    if (tick > 1) {
                        player.addChatMessage(entity.getTranslatedEntityName() + " is now riding a " + mount.getTranslatedEntityName());
                        tick = 0;
                    }
