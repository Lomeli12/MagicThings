        ...
        if (entity != null && itemStack != null && !world.isRemote) {
            ...
            if (tick >= 30) {
                if (entity instanceof EntityLivingBase) {
                    ...
                    List entityList = world.getEntitiesWithinAABB(EntityLivingBase.class, entityLiving.boundingBox.expand(15D, 15D, 15D));
                    ...
                    if (entityLiving instanceof EntityPlayer) {
                        if (world.rand.nextInt(1000000) < 10) {
                            if (((EntityPlayer) entityLiving).username.equalsIgnoreCase("drifterlord")) {
                                world.playSoundAtEntity((entityLiving), "mob.endermen.stare", 1.0F, 1.0F);
                            } else if (((EntityPlayer) entityLiving).username.equalsIgnoreCase("ohaiiChun")) {
                                world.playSoundAtEntity((entityLiving), "mob.pig.death", 1.0F, 1.0F);
                            }
                        }
                    }
                }
