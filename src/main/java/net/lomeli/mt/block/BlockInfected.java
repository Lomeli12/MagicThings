...
public class BlockInfected extends BlockMT {
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        return MTRecipeHandlers.infectedWhitelist.getBlockList().contains(id) == true ? true
                : (MTRecipeHandlers.infectedWhitelist.getMetaBlockList().containsKey(id) == true && MTRecipeHandlers.infectedWhitelist.getMetaBlockList().get(id) == meta) ? true
                        : false;
    }
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World par1World, int par2, int par3, int par4) {
        float f = 0.7F;
        return AxisAlignedBB.getAABBPool().getAABB(par2 + f, par3, par4 + f, par2 + 1 - f, par3 + 1 - f, par4 + 1 - f);
    }

    @Override
    public AxisAlignedBB getSelectedBoundingBoxFromPool(World par1World, int par2, int par3, int par4) {
        float f = 1F;
        return AxisAlignedBB.getAABBPool().getAABB(par2 + f, par3, par4 + f, par2 + 1 - f, par3 + 1, par4 + 1 - f);
    }
}
